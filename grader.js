...
var checkAndDisplay = function(htmlContent, checks) {
    var checkJson = checkHtmlFile(htmlContent, checks);
...
if(require.main == module) {
    program
        ...
        .option('-u, --url <URl>', 'Path to URl html')
        .parse(process.argv);
    ...
    if(program.url !== undefined ){
        restler.get(program.url).on('complete', function(result) {
            if (result instanceof Error) {
                ...
                checkAndDisplay(result, program.checks);
            }
        });
    }
    else {
        var htmlContent = fs.readFileSync(program.file);
        checkAndDisplay(htmlContent, program.checks);
    }
} else {
